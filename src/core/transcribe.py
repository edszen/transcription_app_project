...
from src.core.diarization import apply_diarization, assign_speakers
...
class Transcriber(QObject):
    ...
    def transcribe(self, file_path, use_diarization=True, encrypted_api_key=None):
        try:
            self.cancel_flag = False
            ...
            result = whisperx.align(result["segments"], model_a, metadata, file_path, device, return_char_alignments=False)
            ...
            if use_diarization:
                ...
                logger.info(f"Diarization completed. Result length: {len(diarization_result)}")
                
                # Assign speakers to transcription segments
                self.status_updated.emit("Assigning speakers to transcription segments...")
                result = assign_speakers(result["segments"], diarization_result)
                logger.info(f"Speaker assignment completed. Segments: {len(result)}")
